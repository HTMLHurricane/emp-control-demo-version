...
const AdminUpdateScheduleForm: FC = () => {
  ...
  useEffect(() => {
    if (scheduleForm) {
      form.setFieldValue('name', scheduleForm.name)
      scheduleForm.days.map((day) => {
        form.setFieldValue(
          ['weekDays', day.day, 'startTime'],
          day.time_in?.slice(0, 5),
        )
        form.setFieldValue(
          ['weekDays', day.day, 'endTime'],
  ...
  useEffect(() => {
    if (isSuccess) {
      message.success('График работы успешно отредактирован')
      setIsUpdatingSchedule(false)
    }
    if (isError) {
      message.error('Произошла ошибка во время редактирования графика работы')
